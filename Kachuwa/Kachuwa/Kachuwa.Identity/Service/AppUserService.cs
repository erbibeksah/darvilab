...
namespace Kachuwa.Identity.Service
{
    public class AppUserService : IAppUserService
    {
        ...
        public AppUserService(UserManager<IdentityUser> userManager, RoleManager<IdentityRole> roleManager, 
            IIdentityUserService identityUserService, IIdentityRoleService identityRoleService,IStorageProvider storageProvider, ILogger logger, SessionExt session)
        {
        ...
        public async Task<UserStatus> AddExternalUser(string provider, string providerUserId, List<Claim> claims)
        {
            ...
            try
            {
                ...
                if (existingUser != null)
                {
                    ...
                    objSession.Email = existingUser?.Email?.ToLower();
                    objSession.Username = existingUser.UserName;
                    objSession.UserId = existingUser.Id;
                    objSession.TwoFactorEnabled = existingUser.TwoFactorEnabled;
                    objSession.Roles = existingUser.Roles != null ? existingUser.Roles.Count : 0;
                    _session.Set("_user", objSession);
                    return status;
                }
                            ...
                            using (var dbTran = db.BeginTransaction())
                            {
                                try
                                {
                                    ...
                                    if (roleIds != null && roleIds.Any())
                                        ...
                                        await _identityUserService.AddUserRoles(new int[] { 3 }, identityUser.Id);
                                    ...
                                    status.Message = id.ToString();
                                    CSession objSession = new CSession();
                                    objSession.Email = iuser?.Email?.ToLower();
                                    objSession.Username = iuser.UserName;
                                    objSession.UserId = id;
                                    objSession.TwoFactorEnabled = iuser.TwoFactorEnabled;
                                    objSession.Roles = iuser.Roles != null ? iuser.Roles.Count : 0;
                                    _session.Set("_user", objSession);
                                    return status;
                                }
                                catch (Exception e)
        ...
        public async Task<List<UserInfoView>> GetAllUsers()
        {
            var dbFactory = DbFactoryProvider.GetFactory();
            using (var db = (DbConnection)dbFactory.GetConnection())
            {
                ...
                string sqlQuery = @"
                                    SELECT 
                                        ...
                                        A.Email,
                                        L.LoginProvider
                                    FROM IdentityUser AS I WITH (NOLOCK)
                                    INNER JOIN AppUser AS A WITH (NOLOCK)
                                        ON A.IdentityUserId = I.Id
                                    LEFT JOIN IdentityUserLogin AS L WITH (NOLOCK)
                                        ON L.UserId = I.Id
                                    LEFT JOIN IdentityUserRole AS R WITH (NOLOCK)
                                        ON R.UserId = I.Id";
		        var users = await db.QueryAsync<UserInfoView>(sqlQuery);
		        return users.ToList();
            }
        }
